...
async function getIntroduction(where = "default") {
    ...
    if (local != null && JSON.parse(local).time + 1000 * 60 < new Date().getTime()) {
        ...
        introduction.languages.forEach((language: { url: string; color: string; name: string; }) => {
            languages = `${languages}${introduction.languages.indexOf(language) == introduction.languages.length - 1 ? " en " : " "}<a href='${language.url}'><span class='language' style='color: ${language.color}'>${language.name}</span></a>`;
        });
